...
class _WaitingDisplayState extends ConsumerState<WaitingDisplay> {
        ...
        RichText(
          text: context.trFromObjectToTextSpan(
            TrObject(
              ...
              richTrObjects: [
                RichTrObject(RichTrType.number, value: widget.game.subgameNum),
              ],
            ),
            [],
          ),
      ...
      children: [
        Row(
          children: [
            ...
            const SizedBox(width: 5),
            PlayerIcon(index: widget.game.playerNum - 1, displayNumber: true),
          ],
        ),
  ...
  Widget _buildSinglePlayerIcon(
    ...
    child: Row(
      children: [
        ...
        OwnText(text: displayName, translate: false, ellipsis: true),
        if (trailing != null) ...[
          const Spacer(),
          trailing,
  ...
  Future<void> _addPlayer() async {
    ...
    while (widget.game.players.any(
      (element) => element.id == playerDict[playerIndex],
    )) {
      playerIndex += 1;
      if (!playerDict.containsKey(playerIndex)) return;
    }
    await widget.game.tryAddPlayer(
      Player(id: playerDict[playerIndex]!, displayName: 'Player $playerIndex'),
    );
  }
}
